...
#if defined(NDEBUG)
# error "Siccoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x46a6bf8470e30f47d03528fbfe8bb2ca811e96b3b0d24ff6e6116c68a37494ef");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Siccoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Siccoin Signed Message:\n";
...
{
    // Siccoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Siccoin: 10k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Siccoin: 1 day
static const int64 nTargetSpacing = 5 * 60; // Siccoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Siccoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Siccoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1ef7e0ef089094969807dcd7e16e6bf5f8d1554adbeb8ed51ad530be227e3206");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Baylor defeats TCU in football to clench bowl eligibility 16/Nov/2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e968fa47f1008c60164e85ef1ef3e711aa0581a6424a2a7b589b4eedf757ae4773dc4ad16bed67a8b2c59225e0d812dd6c95c26f1aeed7adb3f8782b27380e3a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1542391772;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084869425;

        if (fTestNet)
        {
            block.nTime    = 1542391772;
            block.nNonce   = 387273678;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xa2cfe04474881df2844bd052aad0e924ee59d2fda243eb140b12204f6faadba0"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Siccoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SiccoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SiccoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SiccoinMiner(CWallet *pwallet)
{
    printf("SiccoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("siccoin-miner");
    ...
    try { loop {
        ...
        printf("Running SiccoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SiccoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SiccoinMiner, pwallet));
}
